...
function header_info {
  clear
  cat <<"EOF"
 ___       __   ________  ________  ___  ___  ___  ___     
|\  \     |\  \|\   __  \|\_____  \|\  \|\  \|\  \|\  \    
\ \  \    \ \  \ \  \|\  \\|___/  /\ \  \\\  \ \  \\\  \   
 \ \  \  __\ \  \ \   __  \   /  / /\ \  \\\  \ \   __  \  
  \ \  \|\__\_\  \ \  \ \  \ /  /_/__\ \  \\\  \ \  \ \  \ 
   \ \____________\ \__\ \__|\________\ \_______\ \__\ \__\
    \|____________|\|__|\|__|\|_______|\|_______|\|__|\|__|
...
function advanced_settings() {
  ...
  if VLAN1=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set a Vlan(leave blank for default)" 8 58 101 --title "VLAN" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VLAN1 ]; then
      VLAN1="Default"
      VLAN=""
  ...
  if (whiptail --backtitle "Proxmox VE Helper Scripts" --title "ADVANCED SETTINGS COMPLETE" --yesno "Ready to create wazuh VM?" --no-button Do-Over 10 58); then
    echo -e "${RD}Creating a wazuh VM using the above advanced settings${CL}"
  else
    header_info
...
if [ ! -f /var/lib/vz/import/wazuh-${VERSION}.ova ];then  
 ...
 msg_info "you already downloaded the file \n"
fi
...
msg_ok "Created Wazuh VM ${CL}${BL}(${HN})"
if [ "$START_VM" == "yes" ]; then
  msg_info "Starting Wazuh VM"
  qm start $VMID
